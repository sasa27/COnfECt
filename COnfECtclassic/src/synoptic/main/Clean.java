...
public class Clean {
	...
	public static void makeStrict(String name, String output, int ecart) {
		//int m = Integer.parseInt(name.substring(name.indexOf("T") + 1, name.indexOf(".")));
		//int n = Integer.parseInt(output.substring(output.indexOf("C") + 1, output.indexOf(".")));
		//int ecart = m-n;
		try {
			String[] transitions = getTransitions(name);
			String[] nodes = getLabels(name);
			...
			for (String edge: transitions) {
				if (edge.contains("call_T")){
					int source = Integer.parseInt(edge.substring(0, edge.indexOf("-")));
					int dest = Integer.parseInt(edge.substring(edge.indexOf(">") + 1, edge.indexOf(" ")));
					call = Integer.parseInt(edge.substring(edge.indexOf("T")+1 , edge.indexOf("]")-1));
					call = call + ecart;
					bw.write(source + "->" + dest + "  [label=\"call_C" + call + "\"];\n");
				}
				else if (edge.contains("return_T")){
					int source = Integer.parseInt(edge.substring(0, edge.indexOf("-")));
					int dest = Integer.parseInt(edge.substring(edge.indexOf(">") + 1, edge.indexOf(" ")));
					call = Integer.parseInt(edge.substring(edge.indexOf("T")+1 , edge.indexOf("]")-1));
					call = call + ecart;
					bw.write(source + "->" + dest + "  [label=\"return_C" + call + "\"];\n");
				}
				else {
					bw.write(edge + "\n");
				}
	...
	public static void hideCallStrict(String name, String output) {
		try {
			String[] transitions = getTransitions(name);
			String[] nodes = getLabels(name);
			BufferedWriter bw = new BufferedWriter(new FileWriter(output));
			...
			for (String edge: transitions) {
				...
				int endLabel = edge.indexOf("\"", beginLabel);
				String label = edge.substring(beginLabel, endLabel); 
				if (label.contains("call_") || label.contains("return_")) {
					change[source] = change[dest];
				}
			}
			...
			while (fds) {
				...
				for (int i = 0; i < change.length; ++i) {	
					int finale = change[i];
					if (finale != change[finale]) {
						System.out.println("pass");
						change[i] = change[finale];
						fds = true;
					}
				}
			}
			...
			for (String edge: transitions) {
				...
				if (label.contains("call_") || label.contains("return_")) {
					...
					String trans = change[source] + "->" + change[dest] + "  [label=\"" + label + "\"]";
					bw.write(trans + "\n");
				}
			}
			bw.write("}\n");
