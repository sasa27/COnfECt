...
public abstract class AbstractMain {
    ...
    public static GraphExportFormatter processArgs(AbstractOptions options)
            throws IOException, URISyntaxException, IllegalArgumentException, IllegalAccessException {
        ...
        if (options.runAllTests) {
            List<String> testClasses = SynopticJar.getTestsInPackage("synoptic.tests.units.");
            testClasses.addAll(SynopticJar.getTestsInPackage("synoptic.tests.integration."));
            SynopticLibTest.runTests(testClasses);
            // Terminate after we are done running tests.
            return null;
        } else if (options.runTests) {
            List<String> testClassesUnits = SynopticJar.getTestsInPackage("synoptic.tests.units.");
        ...
        if (AbstractOptions.plumeOpts.logFilenames.size() == 0) {
            logger.severe(
                    "No log filenames specified, exiting. Specify log files at the end of the command line with no options.");
            return null;
        }

        if (options.dumpIntermediateStages && AbstractOptions.outputPathPrefix == null) {
    ...
    public static File[] getFiles(String fileArg) throws Exception {
        ...
        if (results == null) {
            throw new Exception("Wildcard match failed: "
                    + (dir.isDirectory() ? dir.toString() + " not a directory" : " for unknown reason"));
        }
        return results;
    }
    ...
    public String getIntermediateDumpFilename(String stageName, int roundNum) {
        return AbstractOptions.outputPathPrefix + ".stage-" + stageName + ".round-" + roundNum;
    }
     ...
     * 
     * @param outputProbLabelsCond
     *            Whether or not to output probabilities on the edge labels
     * @param outputCountLabelsCond
     *            Whether or not to output counts on the edge labels
     * @throws IOException
     */
    private <T extends INode<T>> void exportGraph(String baseFilename, IGraph<T> g, boolean outputProbLabelsCond,
            boolean outputCountLabelsCond, boolean imageGenCondition) {
        ...
        try {
            GraphExporter.exportGraph(filename, g, outputProbLabelsCond, outputCountLabelsCond);
        } catch (IOException e) {
            logger.fine("Unable to export graph to " + filename);
        }
        
        /** don't generate the png 
     ...
     */
    public <T extends INode<T>> void exportTraceGraph(String baseFilename, IGraph<T> g) {
        ...
        // format).
        exportGraph(baseFilename, g, false, false, options.dumpTraceGraphPngFile && !options.exportAsGML);
    }

    /**
     * Export g as a non-initial graph.
     */
    public <T extends INode<T>> void exportNonInitialGraph(String baseFilename, IGraph<T> g) {
        ...
        // use the 'dot' command).
        exportGraph(baseFilename, g, options.outputProbLabels, options.outputCountLabels, !options.exportAsGML);
    }
    ...
    private void processPOLog(TraceParser parserIn, List<EventNode> parsedEvents)
        ...
        if (AbstractOptions.separateVTimeIndexSets != null) {
            ...
            for (String strSet : AbstractOptions.separateVTimeIndexSets.split(";")) {
        ...
        // //////////////////
        TemporalInvariantSet minedInvs = minePOInvariants(options.useTransitiveClosureMining, traceGraph);
        // //////////////////
        ...
        logger.info("\tMined " + totalNCwith + " NeverConcurrentWith invariants");
     ...
     * @throws Exception
     */
    static public List<EventNode> parseEvents(TraceParser parser, List<String> logFilenames) throws Exception {
    ...
    static public ChainsTraceGraph genChainsTraceGraph(TraceParser parser, List<EventNode> parsedEvents)
            throws ParseException {
        long startTime = loggerInfoStart("Generating inter-event temporal relation...");
        ChainsTraceGraph inputGraph = parser.generateDirectTORelation(parsedEvents);
        loggerInfoEnd("Generating temporal relation took ", startTime);
        return inputGraph;
    }

    static public DAGsTraceGraph genDAGsTraceGraph(TraceParser parser, List<EventNode> parsedEvents)
            throws ParseException {
        long startTime = loggerInfoStart("Generating inter-event temporal relation...");
        DAGsTraceGraph traceGraph = parser.generateDirectPORelation(parsedEvents);
        loggerInfoEnd("Generating temporal relation took ", startTime);
        return traceGraph;
    }
     ...
     * @return
     */
    public TemporalInvariantSet mineTOInvariants(boolean useTransitiveClosureMining, ChainsTraceGraph traceGraph) {
        return mineTOInvariantsCommon(useTransitiveClosureMining, traceGraph);
    }

    protected TemporalInvariantSet mineTOInvariantsCommon(boolean useTransitiveClosureMining,
        ...
        long startTime = loggerInfoStart("Mining invariants [" + miner.getClass().getName() + "]..");
        TemporalInvariantSet minedInvs = miner.computeInvariants(traceGraph, options.multipleRelations,
     ...
     * @return
     */
    public TemporalInvariantSet minePOInvariants(boolean useTransitiveClosureMining, DAGsTraceGraph traceGraph) {
    ...
    public PartitionGraph createInitialPartitionGraph() throws Exception {
        ...
        if (options.dumpTraceGraphDotFile) {
            logger.info("Exporting trace graph [" + traceGraph.getNodes().size() + " nodes]..");
        ...
        loggerInfoEnd("Creating partition graph took looool ", startTime);
        // //////////////////
    ...
    public static void normalizeTraceGraph(ChainsTraceGraph traceGraph) {
        ...
        for (Trace trace : traceGraph.getTraces()) {
            Set<IRelationPath> subgraphs = trace.getSingleRelationPaths(Event.defTimeRelationStr);
        ...
        for (IRelationPath relationPath : relationPaths) {
            ...
            while (!cur.getAllTransitions().isEmpty()) {
                ...
                cur = cur.getTransitionsWithIntersectingRelations(traceGraph.getRelations()).get(0).getTarget();
            }
            ...
            if (maxTime != null) {
                ...
                logger.fine("Warning: Trace beginning with " + relationPath.getFirstNode()
            ...
            while (!cur.getAllTransitions().isEmpty()) {
                cur.getEvent().setTime(cur.getTime().computeDelta(minTime).normalize(rangeTime));
     ...
     */
    /*protected*/ 
    public void exportGraph(PartitionGraph pGraph) {
        long startTime;
        if (AbstractOptions.outputPathPrefix != null) {
            ...
            logger.info("Exporting took " + (System.currentTimeMillis() - startTime) + "ms");

            // if test generation is enabled, export all bounded, predicted
            // abstract tests
            if (options.testGeneration) {
                Set<AbstractTestCase> testSuite = SynopticTestGeneration.deriveAbstractTests(pGraph);
                int testID = 0;
                for (AbstractTestCase testCase : testSuite) {
                    String baseFilename = AbstractOptions.outputPathPrefix + "-test" + testID;
        ...
        if (options.outputJSON) {
            ...
            JsonExporter.exportJsonObject(AbstractOptions.outputPathPrefix, pGraph);

            logger.info("Exporting JSON object took " + (System.currentTimeMillis() - startTime) + "ms");
        }
        ...
        if (options.outputLTS) {
            ...
            logger.info("Exporting in LTS format took " + (System.currentTimeMillis() - startTime) + "ms");
        }
    }
}
